...
Long division is a machine. You feed it a numerator and a denominator, and it produces digits. One at a time, forever if necessary. The digit it produces at each step depends on one thing: the remainder.

The remainder $r$ enters the machine. The machine computes $\lfloor br/p \rfloor$ and outputs a digit. Then it passes along the new remainder, and the cycle continues. That function, $\delta(r) = \lfloor br/p \rfloor$, is the engine of everything that follows. It is the digit function. It is trivially defined, universally familiar, and easy to underestimate.

## Perfect separation

Some primes enforce a kind of total order on the fractions they govern. Take $p = 7$ in base 10:
...
Six fractions, six repeating blocks, and no two of them ever share a digit at the same position. The six repetends are perfectly separated. Every fraction has its own lane.

Now take $p = 13$. Twelve fractions, but only ten possible digits. Two collisions are forced. The separation breaks.
...
This is not an accident. In base 10, $10 \equiv -1 \pmod{11}$. Multiplying a remainder by 10 is the same as negating it. So the second digit of each repetend comes from the mirror of the first. The digit function forces the two digits to sum to 9: $\lfloor 10r/11 \rfloor + \lfloor 10(11{-}r)/11 \rfloor = 9$. And the fraction $k/11$ paired with $(11{-}k)/11$ swaps the two digits, because complementing $k$ reverses the cycle. 09 becomes 90. 18 becomes 81. The outward path and its mirror.
...
For every digit-partitioning prime, the alignment of the fractional field has the same formula:
...
This means the digit-partitioning condition is not a fact about primes. It is a fact about the interaction between a prime and a coordinate system. The base is not revealing structure. It is imposing a grid, and the structure shows through the grid differently depending on how fine the grid is.
...
Three luminous vertical columns of different heights, representing the primes 3, 7, and 11, standing in a field of fainter, shorter columns (the non-DP primes 13, 17, 19, ...). The three bright columns are separated by clean space. The others crowd together, overlapping, colliding. A faint horizontal line at the golden threshold divides the image: the column for 3 rises above it, the others do not. The geometry should convey: these three primes have room for every digit. The rest do not.

No text, no axes, no labels. The mathematics is in the separation.

Aspect ratio: 1200 x 800 (landscape). High contrast.

**Caption:** A rotating field, cut by the digit function into contiguous regions. Each interval carries its own trajectory. No two paths overlap. Alignment is a consequence of perfect partition.
