...
This site documents a research program in number theory built around the digit function and its collision structure.
...
Several results in this program connect to the zeros of L-functions. These connections are conditional: the collision framework provides equivalences and reductions, not proofs of GRH.

The proved results include the gate width theorem (exactly $b-1$ deranging multipliers at every prime, via an explicit rational family), the collision periodic table (a finite deterministic encoding of collision counts at each lag), a Parseval moment identity giving an exact formula for a weighted second moment of $|L(1,\chi)|^2$, and conditional convergence estimates for the collision fluctuation sum.
...
- [The Collision Periodic Table](https://github.com/alexspetty/nfield/blob/main/publication/the_collision_periodic_table.pdf) (2026, preprint). The periodic table at general lag, L-value structure, effective error terms.
- [Convergence of Collision Fluctuation Sums](https://github.com/alexspetty/nfield/blob/main/publication/convergence_of_collision_fluctuation_sums.pdf) (2026, preprint). Conditional convergence estimates, avoidance deficit, pair decomposition.

## About the Author
![Alexander S. Petty](https://alexpetty.com/content/images/2026/03/asp1.jpg)
Alexander S. Petty is an independent researcher in number theory with several patents pending in computational methods for identity and recursive structure. He lives in Virginia.
