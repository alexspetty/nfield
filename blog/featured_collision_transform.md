...
This paper proves they are not irregular. Three hidden symmetries fall out of one easy observation. Each symmetry is a theorem. Together they explain why the collision invariant cancels across primes at the same boundary where the prime number theorem lives.

The easy observation is the antisymmetry of complement pairs:
...
## A negative result that means something

Here is a small extra. Compute the collision invariant in many bases at once. Sum the values across bases with a fixed weighting. The aggregate signal is *empty*. Individual bases can produce $F(s)$ values ranging from $-1.4$ to $+5.1$, but a weighted sum across bases lands at $-0.01$. The collision invariant's structural content is *base-specific*. Each base sees a different view of the primes. The views do not aggregate into a single universal view; they cancel out.
