...
Now generalize. Replace 3 with any prime $p$. The alignment formula becomes $(2m-1)/(pm-1)$, approaching $2/p$.

For the alignment to ever reach $1/\varphi$, you need $2/p > 1/\varphi$, which forces $p < 2\varphi \approx 3.236$.
...
The golden ratio satisfies $\tau^2 + \tau - 1 = 0$. This polynomial divides the cubic when $p = 3$, and only when $p = 3$. The remainder of the division is $(3 - p)\tau$. It vanishes if and only if $p = 3$. The number 3 is not put in by hand. The polynomial produces it.
...
A single golden spiral, luminous and precise, emerging from a field of faintly glowing prime numbers scattered in darkness. The spiral passes through exactly one point that burns brighter than the rest: the prime 3. Other primes are present but untouched, dimmer, receding. The spiral is not decorative. It is a selection mechanism. The geometry should feel inevitable, like a key fitting a lock.

No text, no axes, no labels. The mathematics is in the selection.

Aspect ratio: 1200 x 800 (landscape). High contrast.

**Caption:** The golden ratio's algebra selects the prime 3 from among all primes. The selection is not numerical. It is structural.
