...
for an explicit constant $\sigma^2$. Third, the periodic
table encodes the values $L(1, \chi)$ for all primitive odd
characters modulo~$b^2$ through the universal formula
$|\widehat{c}(\chi)| =
|B_1(\overline{\chi})| \cdot |L(1,\chi)| / (4\pi)$.
\end{abstract}
\maketitle
...
\begin{proof}
Since $S^{\circ}(p)$ depends only on $p \bmod b^2$
(Theorem~\ref{thm:fingerprint}), the sum decomposes
...
O\!\left(\frac{x}{\log^2 x}\right).
\]
By Parseval on $(\mathbb{Z}/b^2\mathbb{Z})^{*}$:
...
For composite bases, the extraction admits a single
proportionality constant:

\begin{corollary}\label{cor:composite}
For composite bases $b \in \{6, 10, 12, \ldots\}$:
...
\subsection*{The collision oracle}

The periodic table provides $C(b \bmod p)$ in $O(1)$:
look up $\mathcal{T}(p \bmod b^2)$ and add
$\lfloor(p{-}1)/b\rfloor$. The table replaces an $O(p)$
...
For composite bases: $|S_G(\chi)| = |B_1(\overline{\chi})|$,
and the extraction has a single constant $1/(\pi \varphi(b))$.
