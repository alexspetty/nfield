...
\date{April 2024; revised April 2026}
\subjclass[2020]{11A63, 11A07, 11B83}

\begin{document}
\begin{abstract}
For any prime $p > b$, define the collision count
$C(g) = \#\{r : \lfloor br/p \rfloor =
\lfloor b(gr \bmod p)/p \rfloor\}$. We prove that
the set of multipliers $g$ with $C(g) = 0$
(bin derangements) is exactly
$\{-u/(b{-}u) \bmod p : u = 1, \ldots, b{-}1\}$.
In particular, there are exactly $b - 1$ such elements,
independent of the prime. No primitive-root hypothesis
is needed. The key observation is that the map
$r \mapsto br \bmod p$ sends contiguous digit bins to
residue classes modulo~$b$.

When $b$ is a primitive root modulo $p$, the bin
derangements coincide with the zero lags of the
...
For the residue class $R = 0$, this simplifies to
$\overline{C}=Q$ exactly. For $Q = 1$ (equivalently,
$b+1 < p \le 2b+1$), the mean is always~$2$.
...
over digit constraints~\cite{shparlinski}, and classical
properties of positional expansions~\cite{hardy-wright}. The
function partitions $\{1, \ldots, p{-}1\}$ into $b$ bins
$B_0, \ldots, B_{b-1}$, where
\[
B_d = \{r \in \{1, \ldots, p{-}1\} : \delta(r) = d\}.
\]
These bins form a partition by contiguous intervals: the bin sizes are
$\lfloor (d{+}1)p/b \rfloor - \lfloor dp/b \rfloor$,
determined directly by interval arithmetic.

When $b$ is a primitive root modulo $p$, the autocorrelation of
the repetend of $1/p$ at lag~$\ell$ counts positions $j$ where
the digit at position~$j$ equals the digit at position
$j + \ell$:
\[
...
Without the primitive-root hypothesis, the same formula defines
the collision count
\[
C(g) = \sum_{d=0}^{b-1} |B_d \cap g B_d|
\]
for an arbitrary multiplier $g \in \mathbb{F}_p^\times$, and
the algebraic arguments below work directly with $C(g)$.
...
\[
|B_d|
= \left\lfloor \frac{(d+1)p}{b}\right\rfloor
 - \left\lfloor \frac{dp}{b}\right\rfloor .
\]
...
extended to the partition setting: no element remains in its
original bin block.
\end{definition}
...
$\{1,\ldots,p-1\}$. Multiplication by $g$ permutes the nonzero
residues modulo $p$. A bin derangement is a multiplier for which
no bin meets its image.
...
\begin{proof}
For $g \in \mathbb{F}_p^\times$, the condition
$g \in \bigcup_{d=0}^{b-1} B_d/B_d$ means that
$g = xy^{-1}$ for some $x,y \in B_d$, equivalently
$gy = x \in B_d$ for some $y \in B_d$. Thus
$g$ lies in the union if and only if it is not bin-deranging.
By Theorem~\ref{thm:gate-width}, exactly $b-1$ of the $p-1$
units are deranging, so the union has size
$(p-1)-(b-1)=p-b$.
\end{proof}
...
The autocorrelation satisfies a simple double-counting identity
connecting the collision count to the zeroth bin-size moment.
...
R$ bins of size $Q$. Hence
\[
\sum_{d=0}^{b-1} n_d^2 = R(Q+1)^2 + (b-R)Q^2 = bQ^2 + R(2Q + 1).
\]

\begin{theorem}[Mean constructive value]\label{thm:mean}
For $p > b + 1$ (so that constructive multipliers
exist), the mean of $C(g)$ over the $p - b - 1$
constructive multipliers is
\[
\overline{C} = \frac{Q\bigl(b(Q{-}1) + 2R\bigr)}{b(Q{-}1) + R}.
\]
...
\begin{proof}
There are $p - 2$ multipliers $g \ne 1$, of which $b - 1$ have
...
\[
\sum_{g \in \mathbb{F}_p^\times} C(g) - C(1)
= bQ^2 + R(2Q+1) - (p-1)
= bQ^2 - bQ + 2RQ
= Q(b(Q{-}1) + 2R).
\]
...
\item For $Q = 1$ (equivalently, $b+1 < p \le 2b+1$):
      $\overline{C} = 2$.
\item As $Q \to \infty$ (large primes in any class):
      $\overline{C}/Q \to 1$, equivalently
      $\overline{C} = Q + O(1)$.
\end{enumerate}
\end{corollary}
...
(3) Rewrite the mean as
\[
\overline{C}
= Q + \frac{QR}{b(Q{-}1)+R}.
\]
Since $0 \le R < b$, the second term is $O(1)$ and hence
$\overline{C}/Q \to 1$ as $Q \to \infty$.
\end{proof}
...
\toprule
$b$ & $p$ & $p\bmod b$ & $Q$ & $R$ & zeros & $\overline{C}$ actual
& $\overline{C}$ formula \\
\midrule
...
$6$ & $41$ & $5$ & $6$ & $4$ & $5$ & $6.71$ & $6.71$ \\
$6$ & $89$ & $5$ & $14$ & $4$ & $5$ & $14.68$ & $14.68$ \\
$7$ & $17$ & $3$ & $2$ & $2$ & $6$ & $2.44$ & $2.44$ \\
$7$ & $41$ & $6$ & $5$ & $5$ & $6$ & $5.76$ & $5.76$ \\
$7$ & $97$ & $6$ & $13$ & $5$ & $6$ & $13.73$ & $13.73$ \\
$12$ & $17$ & $5$ & $1$ & $4$ & $11$ & $2.00$ & $2.00$ \\
$12$ & $67$ & $7$ & $5$ & $6$ & $11$ & $5.56$ & $5.56$ \\
\bottomrule
\end{tabular}
\caption{Gate width and mean formula verified across bases. The
zero count equals $b - 1$ in every case, and the mean formula is
exact. The third column records the residue class $p \bmod b$.}
...
fingerprint of the prime. The mean intensity satisfies
$\overline{C} = Q + O(1)$, so in particular $\overline{C}/Q \to 1$
for large primes, but the
