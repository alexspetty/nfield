/*
 * overlap_rate.c - What is the rate of spectral repulsion?
 *
 * From the uncertainty data:
 * overlap = sum p_i * q_i where p = |s_hat|^2 / total, q = |P|^2 / total
 *
 * For two independent uniform distributions on n points: overlap = 1/n
 * If overlap < 1/n, the distributions are MORE separated than random.
 *
 * Key question: is n * overlap constant? If so, the repulsion has
 * a stable "repulsion ratio" independent of group size.
 */
...
int main(void) {
    printf("%5s %4s %10s %10s %10s %10s\n",
           "base", "n", "overlap", "1/n", "n*overlap", "ratio");
    ...
    for (int b = 3; b <= 37; b++) {
        ...
        double inv_n = 1.0 / n_odd;
        double n_overlap = n_odd * overlap;

        printf("%5d %4d %10.6f %10.6f %10.4f %10.4f\n",
               b, n_odd, overlap, inv_n, n_overlap, overlap / inv_n);
    }
